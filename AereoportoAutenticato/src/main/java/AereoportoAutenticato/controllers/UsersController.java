package AereoportoAutenticato.controllers;

import AereoportoAutenticato.model.Passeggero;
import AereoportoAutenticato.model.Role;
import AereoportoAutenticato.model.RoleToUtenteForm;
import AereoportoAutenticato.model.Utente;
import com.auth0.jwt.JWT;
import com.auth0.jwt.JWTVerifier;
import com.auth0.jwt.algorithms.Algorithm;
import com.auth0.jwt.interfaces.DecodedJWT;
import com.fasterxml.jackson.databind.ObjectMapper;
import AereoportoAutenticato.services.UserService;
...
public class UsersController {
    ...
    @PostMapping("/utenti")
    public ResponseEntity<Utente> saveUtente(@RequestBody Passeggero passeggero, @RequestParam String id, @RequestParam String password) {

        Utente utente = new Utente(id, passeggero.getEmail(), password, new ArrayList<>());
        URI uri = URI.create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/utenti/" + utente.getUsername()).toUriString());
        return ResponseEntity.created(uri).body(serviceUtente.saveUtente(utente));
    }
